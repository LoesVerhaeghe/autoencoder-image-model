...
for folder in all_image_folders:
    ...
    path_to_folder = f"{path_to_mainfolder}/{folder}/basin5/10x"
    ...
    for image_file in images_list_embeddings:
        ...
        if images_in_folder_count > 0:
            ...
            target_per_date[folder]=df_TSS['error_TSSeff'].loc[folder].item() # Save label per date
    folder_idx_counter += 1 # Move to the next folder's error value
    
feature_folder_map = np.array(feature_folder_map) 
seq_len=3
#### test dataset class
dataset = DailySequenceDataset(features_per_date, target_per_date, seq_len=seq_len, n_images=40)
...
df=pd.read_excel('data/pileaute/settler_filtered_data/filtered_TSS_data.xlsx', sheet_name='TSS_effluent', index_col=0)
df.index=pd.to_datetime(df.index)

# --- Construct Model preds ---
TSS_predictions = pd.Series(
    all_preds+df['MechModelOutput'].values[seq_len-1:],
...
plt.plot(df_TSS['error_TSSeff'].iloc[seq_len-1:], '.-', label='Measurements', color='blue')
