...
def train_contrastive_autoencoder(num_epochs, model, optimizer, device, train_loader, val_loader=None,
    ...
    for epoch in range(num_epochs):
        ...
        train_contrastive_loss_accum = 0
        ...
        for batch in train_loader:
            ...
            # Forward pass
            x1_recon = model(x1)
            z1 = model(x1, get_projected=True)
            x2_recon = model(x2)
            z2 = model(x2, get_projected=True)

            #loss = loss_fn(outputs, batch_data)
            total_loss, mse_loss, ssim_loss, contrastive_loss = loss_fn(x1, z1, x1_recon, x2, z2, x2_recon)
            ...
            train_contrastive_loss_accum += contrastive_loss.item()
        ...
        avg_train_contrastive_loss = train_contrastive_loss_accum / len(train_loader)
        ...
        log_dict['train_contrastive_loss_per_epoch'].append(avg_train_contrastive_loss)
        ...
        avg_val_contrastive_loss = float('nan')
        ...
        if val_loader is not None:
            ...
            with torch.no_grad(): # disable gradient computation during validation
                for batch in val_loader:
                    ...
                    val_contrastive_accum += contrastive_loss.item()
                    torch.cuda.empty_cache()
            ...
            avg_val_contrastive_loss = val_contrastive_accum / len(val_loader)
            ...
            log_dict['val_contrastive_loss_per_epoch'].append(avg_val_contrastive_loss)
        ...
        if not skip_epoch_stats:
            ...
            print(f'  Train Loss: Total={avg_train_total_loss:.4f} (MSE={avg_train_mse_loss:.4f}, SSIM={avg_train_ssim_loss:.4f}, C={avg_train_contrastive_loss:.4f})')
            if val_loader is not None:
                print(f'  Val Loss  : Total={avg_val_total_loss:.4f} (MSE={avg_val_mse_loss:.4f}, SSIM={avg_val_ssim_loss:.4f}, C={avg_val_contrastive_loss:.4f})')
            else:
                print()
    ...
    if plot_losses_path is not None:
        ...
        plt.plot(log_dict['train_contrastive_loss_per_epoch'], '.-', label='Contr. train loss')
        ...
        plt.plot(log_dict['val_contrastive_loss_per_epoch'], '.-', label='Contr. val loss')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend()
