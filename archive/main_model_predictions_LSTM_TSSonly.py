...
class DailySequenceDataset(Dataset):
    def __init__(self, target_per_day, seq_len=3):
        '''
        ...
        self.dates = sorted(list(set(target_per_day.keys()))) #only use dates that have both features and a target
    ...
    def create_sequences(self):
        ...
        for i in range(self.seq_len - 1, len(self.dates)):
            ...
            samples.append((prev_targets, target))
        return samples
    ...
    def __getitem__(self, idx):
        prev_targets, target = self.samples[idx]
...
class FullPipelineModel(nn.Module):
    ...
    def forward(self, prev_targets):
        # Pass through your LSTM
        return self.lstm_model(prev_targets.unsqueeze(-1))
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, criterion, epochs=20):
    ...
    for epoch in range(epochs):
        ...
        model.train()
        train_loss = 0
        for prev_targets, y in train_loader:
            prev_targets, y = prev_targets.to(device).float(), y.to(device).float()
            optimizer.zero_grad()
            preds = model(prev_targets)
            loss = criterion(preds, y)
            loss.backward()
            optimizer.step()
            train_loss += loss.item() * prev_targets.size(0)
        ...
        val_loss = 0
        with torch.no_grad():
            for prev_targets, y in val_loader:
                prev_targets, y = prev_targets.to(device).float(), y.to(device).float()
                preds = model(prev_targets)
                loss = criterion(preds, y)
                val_loss += loss.item() * prev_targets.size(0)
...
seq_len=5
#### test dataset class
dataset = DailySequenceDataset(target_per_date, seq_len=seq_len)
...
for i, (prev_targets, target) in enumerate(dataset.samples):
...
trained_model.eval()
with torch.no_grad():
    for prev_targets, y in full_loader:
        prev_targets, y = prev_targets.to(device).float(), y.to(device).float()
        preds = trained_model(prev_targets)
